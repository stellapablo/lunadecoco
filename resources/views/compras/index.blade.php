    ...
    <div class="row">
        <div class="col-sm-12">
            <div class="panel panel-warning">
                <div class="panel-heading">Movimientos de Compras
                    ...
                    <table id="myTable" class="table table-striped">
                        ...
                        @foreach($compras as $row)
                            <tr>
                                ...
                                <td>{{ $row->cantidad }}</td>
                                <td>${{ $row->cantidad * $row->producto->precio }}</td>
                                <td>{{ \Carbon\Carbon::parse($row->created_at)->format('d/m/Y h:m') }}</td>
                                <td>
                                   {{ getEstado($row->pedido_id) }}
                                </td>
                                <td>
                                    <a href="{{ route('compras.finalizar',$row->id) }}">
                                        <div class="col-sm-6 col-md-4 col-lg-3"><i title="Finalizar Pedido" class="ti-check"></i>
                                        </div>
                                    </a>
                                </td>
                            </tr>
